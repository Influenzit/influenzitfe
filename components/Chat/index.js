...
const Messages = ({ serviceId }) => {
  ...
  const handleEmojiClick = (emojiData, event) => {
    ...
    if (
      messageBoxRef.current.lastChild &&
      messageBoxRef.current.lastChild.nodeName === "BR"
    ) {
      messageBoxRef.current.lastChild.remove();
    }
    if (
      messageBoxRef.current.lastChild &&
      messageBoxRef.current.lastChild.nodeName === "DIV"
    ) {
      if (
        messageBoxRef.current.lastChild.lastChild &&
        messageBoxRef.current.lastChild.lastChild.nodeName === "BR"
      ) {
              ...
              <MessagesContB ref={messagesRef}>
                {messages.map((val, i) => (
                  <MessageCard key={i} isOwn={val.is_own}>
                    ...
                    <MessageContent>
                      <h2>
                        {val.from_user?.firstname} {val.from_user?.lastname}{" "}
                        <span>{val.created_at}</span>
                      </h2>
                      <div>{HTMLReactParser(val.text)}</div>
                    </MessageContent>
                  </MessageCard>
                ))}
              </MessagesContB>
              <Editor>
                ...
                >
                  <MessageInput
                    ...
                    contentEditable
                    showPlaceholder={!!messageContent}
                    onInput={handleInput}
                    ref={messageBoxRef}
                  ></MessageInput>
                </div>
...
Messages.getLayout = (page) => <LandingLayout>{page}</LandingLayout>;
