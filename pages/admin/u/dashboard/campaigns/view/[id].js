...
import { createAdminCampaignMilestone, getAdminCampaign, updateAdminCampaign, updateAdminCampaignMilestone } from '../../../../../../api/admin'
...
const CampaignView = () => {
  ...
  const { data, refetch } = useQuery(["get-admin-campaign"], async () => {
      return await getAdminCampaign(id);
  }, {
      enabled: false,
      staleTime: Infinity,
      retry: false,
  ...
  const createMilestone = useMutation( milestoneData => {
      return createAdminCampaignMilestone(id, milestoneData);
  }, {
  ...
  const updateMilestone = useMutation( milestoneData => {
    return updateAdminCampaignMilestone(id, milestoneData.data, milestoneData.id);
}, {
    onSuccess(successRes) {
        const res = successRes.data;
        refetch();
  ...
  const updateCampaignMutation = useMutation( campaignData => {
      return updateAdminCampaign(id, campaignData);
  }, {
      onSuccess(successRes) {
          const res = successRes.data;
          refetch();
