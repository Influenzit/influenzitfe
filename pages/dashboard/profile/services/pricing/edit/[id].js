...
import { createCertifications, createServicePackage, createServices, createSkills, deleteCertification, deleteService, deleteSkill, getCertifications, getService, getServices, getSkills, updateCertifications, updateServicePackage, updateServices, updateSkills } from '../../../../../../api/influencer'
...
const Services = () => {
    ...
    const { data: serviceData, refetch: refetchServiceData } = useQuery(["get-service"], async () => {
        ...
        onSuccess(res) {
            ...
            res.data.data.packages.forEach((val) => {
              if(val.name === "Basic") {
                setBasic({
                  name: "Basic",
                  ...
                  features: val.features.map(feat => ({name: feat.name})),
                })
              }
              if(val.name === "Premium") {
                setPremium({
    ...
    const createPackagesMutation = useMutation( packageData => {
        return updateServicePackage(id, packageData);
    }, {
        onSuccess(successRes) {
            ...
            if(res.errors || res.status === "error" || res.message === "Unauthenticated.") {
                ...
                dispatch(setLoading(false));
                toast.toast.success("FAQ updated successfully", {
    ...
    const handleCreatePackage = () => {
        ...
        if(filteredData.length < 1) {
            ...
            createPackagesMutation.mutate(filteredData);
        }
    }
