...
const Requests = () => {
  ...
  const getRequirement = (name) => {
    return JSON.parse(
      request?.requirements.filter((val) => val.name === name)[0]?.value ?? "[]"
    );
  };
  useEffect(() => {
    if (id) {
          ...
          <Right>
            <RWrapper>
              <RCard>
                ...
                <p>
                  ₦ {request?.amount_start} - ₦ {request?.amount_end}
                </p>
              </RCard>
              <RCard>
                ...
                <div id="requirements">
                  ...
                  <div>
                    <p>Followers</p>
                    <p>
                      {getRequirement("followers")[0]} -{" "}
                      {getRequirement("followers")[1]}{" "}
                    </p>
                  </div>
                  <div>
                    <p>Engagement</p>
                    <p>
                      {getRequirement("engagement_rate")[0]}% -{" "}
                      {getRequirement("engagement_rate")[1]}%
                    </p>
                  </div>
                  <div>
                    <p>Audience Country</p>
                    <p>
                      {getRequirement("country").map((val) => {
                        return `${val}, `;
                      })}
                    </p>
                  </div>
                ...
                {request?.business_id && (
                  <ContinueBtn
                    onClick={() =>
                      router.push(`/business/${request?.business_id}`)
                    }
                  >
...
Requests.getLayout = (page) => <LandingLayout>{page}</LandingLayout>;
