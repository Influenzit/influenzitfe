...
export const updateAdminCampaignMilestone = (id, data, milestoneId) => {
  return axiosInstance().patch(
    `/admin/campaigns/${id}/milestones/${milestoneId}`,
    data
  );
};
...
export const getAllUsersWithoutPagination = (query) => {
  return axiosInstance().get(`/admin/accounts/without-pagination/data${query}`);
};
...
export const getWalletTransactions = (id, query) => {
  return axiosInstance().get(
    query ? query : `/admin/accounts/${id}/wallet-transactions?paginate=20`
  );
};
export const getWalletRequests = (id, query) => {
  return axiosInstance().get(
    query
      ? query
      : `/admin/accounts/${id}/wallet-withdrawals?status=pending&paginate=20`
  );
};
...
export const getWithdrawalRequests = (query, status) => {
  return axiosInstance().get(
    query
      ? query
      : `/admin/wallets/withdrawal-requests?paginate=20&status=${status}`
  );
};
...
export const getAccountActivities = (query) => {
  return axiosInstance().get(
    query ? query : "/admin/logs/activities?paginate=20"
  );
};
...
export const getAdminCampaignRequests = (url) => {
  return axiosInstance().get(
    url
      ? url.includes("/")
        ? url
        : `/admin/campaign/requests/pendings${url}`
      : `/admin/campaign/requests/pendings`
  );
};
